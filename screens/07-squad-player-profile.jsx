...
const DIFF_COLOR = [null,"#16A34A","#65A30D","#D97706","#EA580C","#DC2626"];

function FormBar({ form, pos }) {
...
function FixturePill({ fix }) {
  return (
    <div style={{
      padding:"4px 8px", borderRadius:7,
      background: `${DIFF_COLOR[fix.diff]}18`,
      border: `1px solid ${DIFF_COLOR[fix.diff]}33`,
      ...
      <span style={{ width:6, height:6, borderRadius:"50%", background:DIFF_COLOR[fix.diff], flexShrink:0 }} />
    </div>
  );
}
...
function PlayerProfile({ player, onClose }) {
  ...
  const gwMax = Math.max(...player.form);
        ...
        <div style={{ flex:1, padding:"20px 24px", overflowY:"auto" }}>
          ...
          {tab === "overview" && (
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              ...
              <div style={{ background:"white", borderRadius:12, border:"1.5px solid #EDE5D8", padding:"16px 18px" }}>
                ...
                <FormBar form={player.form} pos={player.pos} />
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[
                  ...
                  { label:"Interceptions",value:player.stats.interceptions, hero: player.hero },
                  { label:"Tackles",      value:player.stats.tackles       },
                  { label:"Saves",        value:player.stats.saves, show: player.pos==="GK" },
                  { label:"Mins Played",  value:player.stats.minsPlayed    },
          ...
          {tab === "stats" && (
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {[
                { cat:"Attacking",  stats:[
                  { label:"Goals",             val:player.stats.goals,        max:25 },
                  { label:"Assists",           val:player.stats.assists,       max:15 },
                  { label:"Key Passes",        val:player.stats.keyPasses,     max:70 },
                ]},
                { cat:"Defensive",  stats:[
                  ...
                  { label:"Clean Sheets",      val:player.stats.cleanSheets,   max:15 },
                ]},
                { cat:"Appearance", stats:[
                  { label:"Mins Played",       val:player.stats.minsPlayed,    max:2700 },
                ]},
              ].map(section => (
              ...
              {player.fixtures.map((f,i) => (
                <div key={i} style={{ background:"white", border:"1.5px solid #EDE5D8", borderRadius:11, padding:"14px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <div style={{ width:8, height:8, borderRadius:"50%", background:DIFF_COLOR[f.diff] }} />
                    <div>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    <div style={{ display:"flex", gap:2 }}>
                      {[1,2,3,4,5].map(d => (
                        <div key={d} style={{ width:8, height:8, borderRadius:2, background: d<=f.diff ? DIFF_COLOR[f.diff] : "#F0E8DC" }} />
                      ))}
                    </div>
                    <span style={{ fontFamily:"'DM Mono', monospace", fontSize:9, color:DIFF_COLOR[f.diff], letterSpacing:"0.06em" }}>
                      {["","Easy","Favourable","Medium","Tough","Very Hard"][f.diff]}
                    </span>
                  </div>
                </div>
              ))}
...
export default function App() {
  ...
  const [view, setView]     = useState("squad");   // squad | stats
  const [posFilter, setPosFilter] = useState("ALL");

  const starters = SQUAD.filter(p => p.starting);
  const bench    = SQUAD.filter(p => !p.starting).sort((a,b) => a.bench - b.bench);
      ...
      <nav style={{
        ...
        <div style={{ display:"flex", gap:28 }}>
          {["Home","Draft","Scoring","Live","League"].map(item => (
            <span key={item} style={{ fontFamily:"'DM Mono', monospace", fontSize:11, letterSpacing:"0.09em", textTransform:"uppercase", color:"#A89880", cursor:"pointer" }}>{item}</span>
          ))}
        </div>
      ...
      <div style={{ maxWidth:1060, margin:"0 auto", padding:"40px 40px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:28 }}>
          ...
          <div style={{ display:"flex", gap:4, background:"#F5EFE8", padding:4, borderRadius:10 }}>
            {[{id:"squad",label:"Squad View"},{id:"stats",label:"Stats View"}].map(v => (
        ...
        <div style={{ display:"flex", gap:12, marginBottom:28 }}>
          {[
            { label:"Season Points",  value: totalPts.toLocaleString(),  sub:"Starting XI total" },
            { label:"GW28 Points",    value: totalGWPts,                  sub:"This gameweek"      },
            { label:"Squad Size",     value: `${SQUAD.length}/15`,        sub:"Players registered" },
            { label:"Best Player",    value: topScorer.short,             sub:`${topScorer.seasonPts} pts · ${topScorer.pos}`, highlight:true },
            { label:"Injuries",       value: injured > 0 ? `${injured} doubt` : "All clear", sub: injured > 0 ? "Check fitness" : "Full squad available", warn:injured>0 },
          ].map(s => (
            <div key={s.label} style={{
          ...
          <div>
            ...
            <div style={{ background:"white", borderRadius:16, border:"1.5px solid #EDE5D8", overflow:"hidden" }}>
              ...
              {filteredSquad.map((p,i) => (
                ...
                onMouseOver={e=>e.currentTarget.style.background="#FFF7F4"}
                onMouseOut={e=>e.currentTarget.style.background=p.hero?"#FFF9F7":"white"}
                >
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  {[p.gwPts, p.seasonPts, p.stats.goals, p.stats.assists, p.stats.interceptions, p.stats.tackles].map((v,ci) => (
                    <span key={ci} style={{ fontFamily: ci<2?"'Playfair Display', serif":"'DM Mono', monospace", fontSize: ci<2?16:12, fontWeight: ci<2?700:400, color: ci===0&&p.hero?"#FF5A1F": ci===4&&p.hero?"#FF5A1F":"#A89880", textAlign:"center" }}>{v}</span>
                  ))}
                </div>
              ))}
            </div>
