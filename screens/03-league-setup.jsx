...
const DRAFT_ORDERS = ["Snake", "Linear", "Auction", "Randomised Snake"];
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div>
        ...
        <input
          value={data.teamName}
          onChange={e => set({...data, teamName: e.target.value})}
...
function StepScoring({ data, set }) {
  ...
  const updateScore = (label, pos, val) => {
    const updated = {
      ...data.scores,
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap" }}>
        {[
          ...
          onMouseOver={e=>{e.currentTarget.style.borderColor="#FF5A1F";e.currentTarget.style.color="#FF5A1F"}}
          onMouseOut={e=>{e.currentTarget.style.borderColor="#EDE5D8";e.currentTarget.style.color="#A89880"}}
          >{p.label}</button>
        ))}
      </div>
                ...
                {POSITIONS.map(pos => {
                  ...
                  return (
                    <input key={pos} value={v ?? 0}
                      onChange={e => updateScore(cat.label, pos, e.target.value)}
                      style={{
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        {[
          ...
          { label:"Transfers", value: { none:"No Transfers", waiver:"Waiver Wire", free:"Free Agency", trade:"Trade Window" }[data.teams.transfers] || "None", sub:"Season format" },
...
export default function App() {
  ...
  const [basics, setBasics]   = useState({ name:"", teamName:"", privacy:"private", format:"h2h" });
  const [teams,  setTeams]    = useState({ teams:10, squadSize:15, startingXI:11, bench:4, formation:"Any", transfers:"waiver" });
  const [draft,  setDraft]    = useState({ draftType:"snake", pickTime:"60s", autopick:"best" });
  const [scoring,setScoring]  = useState({ scores:{ ...DEFAULT_SCORES } });
  ...
  if (launched) {
    return (
      <div style={{ minHeight:"100vh", background:"#FAF7F2", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:24 }}>
        <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Mono:wght@400;500&family=Playfair+Display:ital,wght@0,700;0,900;1,700&family=DM+Sans:wght@300;400;500;600&display=swap');`}</style>
        <div style={{
        ...
        <div style={{ textAlign:"center" }}>
          <h1 style={{ fontFamily:"'Playfair Display', serif", fontSize:42, fontWeight:900, color:"#1C1410", marginBottom:8 }}>
            <span style={{ fontStyle:"italic", color:"#FF5A1F" }}>{basics.name}</span> is live.
          </h1>
          <p style={{ fontFamily:"'DM Mono', monospace", fontSize:12, color:"#A89880", letterSpacing:"0.08em" }}>
            Invite link copied · Waiting for {teams.teams} managers to join
          </p>
        </div>
        <button onClick={() => { setLaunched(false); setStep(1); setBasics({name:"",teamName:"",privacy:"private",format:"h2h"}); }} style={{
          marginTop:8, padding:"12px 28px", borderRadius:10,
          border:"none", background:"linear-gradient(135deg,#FF5A1F,#E8400A)",
          color:"white", fontFamily:"'DM Mono', monospace", fontSize:12,
          letterSpacing:"0.08em", cursor:"pointer",
          boxShadow:"0 4px 16px rgba(255,90,31,0.3)",
        }}>Create Another League</button>
      </div>
    );
  }
